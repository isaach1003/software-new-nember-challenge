...
### Required Software
Below are the required softwares, as well as the versions of them used in the project.
Python (3.13), pandas(2.3.3), numpy(2.3.4)

### Purpose & Use
The purpose of this code is to take a csv file and turn it into a readble JSON output.
...
lap time, sectors, driver names, and lap numbers. 
...
Currently, the code reformats the floats for the JSON output, and makes them round to the 3rd place. If you wish to change rounding for all numbers, go to the ffmt function and switch the default value for the parameter 'decimals'. Alternatively, you can change the rounding of just one value by passing your desired rounding to the function call.
...
The test cases are also available for download and use, but if you do not wish to use those it is neccessary you comment out the code between the comments stand and end of test files.
